...
export const uploadStorage = async (uid: string, id: string, uri: string) => {
  console.log('스토리지 생성');
  return await reference.ref(`${uid}/korea/${id}`).putFile(uri);
};

export const downloadStorage = async (uid: string, id: string) => {
  console.log('스토리지 읽기');
  return await reference.ref(`${uid}/korea/${id}`).getDownloadURL();
};

export const deleteStorage = async (uid: string, id: string) => {
  console.log('스토리지 삭제');
  return await reference.ref(`${uid}/korea/${id}`).delete();
};

export const deleteAllStorage = async (uid: string) => {
